...
public class JyperionListener  implements ITestListener {
		...
		if (throwable != null) {
			...
			/*Paragraph excep = new Paragraph(
					new Chunk(throwable.toString(), 
							...
							setLocalGoto("" + throwable.hashCode()));*/
			/*
			 * Modified below for adding screenshot link in the pdf
			 */
			imagePath = fileSeperator + "Screenshots"
					+ fileSeperator + "Results" + fileSeperator + testClassName
					+ fileSeperator
	        ...
	        imdb.setAction(new PdfAction("file:///"+System.getProperty("user.dir")+imagePath));
	        Paragraph  excep = new Paragraph(
	            throwable.toString());
	        excep.add(imdb);
	...
	public void onStart(ITestContext context) {
		...
		log("onStart("+context+")");
		try {
			PdfWriter.getInstance(this.document, new FileOutputStream(context.getName()+datetime.getDateTime()+".pdf"));
		} catch (Exception e) {
			e.printStackTrace();
		}
		this.document.open();
