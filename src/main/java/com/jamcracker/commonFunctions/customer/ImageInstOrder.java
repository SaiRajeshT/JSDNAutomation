...
public class ImageInstOrder extends TestBase {
	...
	public void instOrder(InstFromImageOrder instorderinfo) {
		...
		explicitWaitToClickable(objImagesPage.goButton);
		...
		case "aws":
			try {
				...
				HandleDropDown.selectDDLByVisibletext(objStackLaunchPage.imageDropDown, instorderinfo.getImageName());
				...
				HandleDropDown.selectDDLByVisibletext(objStackLaunchPage.familyTypeDropDown, instorderinfo.getType());
				...
				HandleDropDown.selectDDLByVisibletext(objStackLaunchPage.networkDropDown, instorderinfo.getNetwork());
				...
				HandleDropDown.selectDDLByVisibletext(objStackLaunchPage.subnetDropDown, instorderinfo.getSubNetName());
		...
		case "windowsazure":
			try {
				...
				HandleDropDown.selectDDLByVisibletext(objStackLaunchPage.publicIPDropdown, instorderinfo.getPublicIp());
				if (HandleDropDown.getSelectedValue(objStackLaunchPage.publicIPDropdown)
						.equalsIgnoreCase(instorderinfo.getPublicIp())) {
					Reporter.log(instorderinfo.getPublicIp() + "  Selected for public IP ");
				} else {
...
//			for (SecurityGroup secGroup : instorderinfo.getSecurityGroups()) {
		...
		try {
							...
							// out or the condition satisfied
			{
				...
				if ((System.currentTimeMillis() / 1000) - startTime > timeout) {
					...
					Reporter.log(
							"<p Style ='color:Red'> Waited for"+timeout+" Seconds. Stack did not go to running please look in to the issue.</p>");
					Assert.fail();
				}
