...
public class TC004StoreBranding extends TestBase {
	...
	@DataProvider(name = "storeDetails")
	public static String[] getBrandingData() {
		ArrayList<String> emailist = ExcelcolumnReader.extractExcelContentByColumnIndex("TestData.xls",	"Store Creation Sheet", 15);
		String[] emailistData = emailist.toArray(new String[emailist.size()]);
		ArrayList<String> companyName = ExcelcolumnReader.extractExcelContentByColumnIndex("TestData.xls", "Store Creation Sheet", 2);
		String[] companyNameData = companyName.toArray(new String[companyName.size()]);
	...
	@Test(dataProvider = "storeDetails")
	public void storeBranding(String storeDetails) throws Exception

	{
		String[] storeData = storeDetails.split(",");
		String storeAdminEmail = storeData[0];
		String companyName = storeData[1];
		String companyAcronym = storeData[2];
		...
		StoreEmailNotificationSetUp.getInstance().emailPageSetup(emailSignature, storeRegisterEmail, emailId, notificationFromAddress, supportEmailSignature, contentRequestEmail, supportFromAddress);
		...
		SetUpBillingOptions.getInstance().testSetUpBillingOptions(storeCurrency, paymentMethods, creditCardPaymentGateway, payPalPaymentGateway, daysBetweenRetries, numberOfPaymentAttempts, defultPaymentMethod, paymentDueAlert, numberOfAlerts, intervalBwnAlert, "testSetUpBillingOptions");
		// objSetUpPage.preferencesLink.click();

		// Reading the data from excel sheet to configure prefernece page and
		// calling StorePrefrence method
		...
		StorePreferences.getInstance().storePreferences(deleteCompany, suspendCompany, proxyCustomer, enableSelfOrder, showPublisherInfo, manageCommercialContent, showAnalyticStatistics, overrideServiceDependency, itemLevelSAD, accessHelpDesk);
